...
import Common.System (putTextLn, repeatUntilValid, convertText, toNonNega2Decimals)
...
readQuantity prompt = repeatUntilValid convertText prompt "Please enter a valid quantity."
...
program = do
  ...
  where
    ...
    display :: Receipt -> AppType ()
    display (subtotal, tax, total) = do
